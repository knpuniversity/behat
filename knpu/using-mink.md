...
Behat and everything else so we can focus on just how *it* works.
...
To execute this, head over to the terminal and run `php mink.php`. Look at that:
it's printing out 200 and a slightly different URL than we put in our code.
...
`$page = $session->getPage()`. I want you to think of this as the JQuery object
or the DOM. Anything you can do with JQuery - like select elements, click links
and fill out fields - you can do with the page. Less impressively, it also knows
the HTML of whatever page we're currently on.
...
`var_dump(substr($page->getText()), 0, 75);`. Run that again in the terminal.
Now we see the thrilling text of: "Park Pedia - Jurassic Park, Dinosaurs, Stephen Spielberg".
There's some weird `a:lang` code stuff on the end. Open up the source on the page.
